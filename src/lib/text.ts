...
export class Text extends Element {
  ...
  layout(constraint: Constraint): Size {
    this.dirtyCache(() => {
      ...
      const ctx = this.getLayer().getContext();
      this.texts = getWordWraps(ctx, constraint.maxWidth, this.text);
  ...
  draw() {
    ...
    if (this.font) {
      ctx.font = generateFont(this.root.font, this.font);
    }
    ...
    this.texts.forEach((v, index) => {
      ctx.fillText(v.text, 0, textHeight * (index + 1) - top);
    });
  }
