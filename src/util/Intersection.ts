...
type IntersectionStatus = 'Intersection' | 'Coincident' | 'Parallel' | undefined;
   ...
   */
    static isPointInPolygon(point: Point, points: Point[]) {
        ...
        for (let index = 0; index < points.length; index++) {
            const inter = this.intersectSegmentSegment(// polygon side
                points[index], points[(index + 1) % points.length], // ray
                point, other);
